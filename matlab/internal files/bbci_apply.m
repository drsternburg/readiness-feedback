...
while run,
  ...
  % Add timestamp when matlab receives the data 
  data.source.timestamp = datestr(now, 'HH:MM:SS.FFF');
  ...
  for ic= 1:length(bbci.control),
    ...
    for ev= 1:length(events),
      ...
      data.control(ic)= ...
          bbci_apply_evalControl(cfy_out, data.source.timestamp, data.control(ic), ...
