function control= bbci_apply_evalControl(cfy_out, timestamp, control, bbci_control, event, marker)
...
if isempty(bbci_control.fcn),
  ...
  if nargout(bbci_control.fcn)==1,
    ...
    control.packet= bbci_control.fcn(cfy_out, timestamp, event_info, ...
                                     bbci_control.param{:});
  else
...
if isnumeric(control.packet) && ~isempty(control.packet),
  control.packet= {'cl_output', control.packet, 'emg', 0};
end
