...
filename_eeg = sprintf('%s\\%s\\%s_%s_%s.mat',BTB.MatDir,ds_name,session_name,phase_name,subj_code);
filename_mrk = sprintf('%s_mrk.mat',filename_eeg);

if not(exist(filename_eeg,'file')) % because for one or two participants we didn't record RT data
    mrk = [];
    cnt = [];
    mnt = [];
    return
